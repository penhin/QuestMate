...
def search_planner_system_prompt() -> str:
    return (
        ...
        "Return only compact JSON with keys: intent, safety_refusal, version_sensitive, requires_relation_verification, named_entity_groups, aliases, queries, answer_requirements, missing_info. "
        ...
        "answer_requirements must contain 1 to 4 short, question-derived obligations that the final answer must resolve. "
        "Use the player's wording for the requested outcome, condition, relationship, or ordered action; do not add facts, guesses, or generic guide sections. "
...
def answer_system_prompt() -> str:
    return (
        ...
        "Use an empty-claim conservative block only for a stated unresolved gap, or when the evidence policy forbids "
        "a concrete answer. "
