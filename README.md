...
在 Planning 之后，Task Router 只依据结构化 `SearchPlan` 生成 `TaskRouteDecision`：任务、
...
After Planning, the Task Router produces a `TaskRouteDecision` solely from the
...
- `task_router/`：`SearchPlan` 到 `TaskRouteDecision` 的结构化任务路由 / structured task routing from `SearchPlan` to `TaskRouteDecision`.
